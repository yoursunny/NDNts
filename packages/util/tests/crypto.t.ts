...
test("timingSafeEqual", () => {
  ...
  expect(timingSafeEqual(ab, ab)).toBeTruthy();
  expect(timingSafeEqual(ab, new Uint8Array(ab))).toBeTruthy();
  expect(timingSafeEqual(ab, new Uint8Array(ab, 1))).toBeFalsy();
  expect(timingSafeEqual(new Uint8Array(ab, 0, 4), new Int16Array(ab, 6))).toBeTruthy();
