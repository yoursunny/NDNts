...
* `--no-metadata` disables publishing current version as metadata packet. This is ignored with `--ver=none`.
...
* `--ver=metadata` (default) sends an metadata discovery Interest to discover version.
...
# consumer: perform version discovery via Metadata Retrieval Protocol
